    ...
    def OnDraw(self):
        ...
        with self._cull:
            ...
            for tris, normals, colors, count in self.triangles:
                ...
                triangles = np.hstack([triangles_vec3, ones]).reshape(-1, 3, 4)
                tri_normals = np.hstack([tri_normals_vec3, ones]).reshape(-1, 4)

                # Triangles SSBO (binding = 0)
                tri_ssbo = GL.glGenBuffers(1)
                GL.glBindBuffer(GL.GL_SHADER_STORAGE_BUFFER, tri_ssbo)
                GL.glNamedBufferData(tri_ssbo, triangles.nbytes, triangles, GL.GL_STATIC_DRAW)
                ...
                GL.glNamedBufferData(tri_normals_ssbo, tri_normals.nbytes, tri_normals, GL.GL_STATIC_DRAW)
                ...
                GL.glNamedBufferData(tri_colors_ssbo, tri_colors.nbytes, tri_colors, GL.GL_STATIC_DRAW)
                ...
                zero = np.array([0], dtype=np.uint32)
                GL.glNamedBufferData(counter_buf, zero.nbytes, zero, GL.GL_DYNAMIC_COPY)
                ...
                GL.glNamedBufferData(idx_ssbo, visible_indices.nbytes, visible_indices, GL.GL_DYNAMIC_COPY)
                ...
                GL.glNamedBufferData(depth_ssbo, visible_depths.nbytes, visible_depths, GL.GL_DYNAMIC_COPY)
                ...
                GL.glNamedBufferData(out_vbo, out_vertex_count * 16, None, GL.GL_DYNAMIC_COPY)
                ...
                GL.glNamedBufferData(out_nbo, out_vertex_count * 16, None, GL.GL_DYNAMIC_COPY)
                ...
                GL.glNamedBufferData(out_cbo, out_vertex_count * 16, None, GL.GL_DYNAMIC_COPY)
                ...
                GL.glDeleteBuffers(1, [counter_data])
