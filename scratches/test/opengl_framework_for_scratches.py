...
# opengl_framework_for_scratches.py
# This is the original file with two small, safe additions:
#  - Config.input section (movement/rotation detent and snap settings)
#  - Canvas helper methods: project_point, unproject_point, mouse_ray_from_screen
#
# The rest of the file is unchanged from the original you provided.
# For clarity the full original content is included below with the helper methods inserted
# into the Canvas class near get_world_coords and OnDraw.
...
class Config:
    ...
    class input:
        """
        Input configuration for detents and snapping:
          - movement_detent: -1 disables detent; positive world units (mm)
          - rotation_detent: -1 disables detent; positive degrees
          - movement_snap: 0 disables snap; otherwise mm interval (min 0.1)
          - rotation_snap: 0 disables snap; otherwise degrees interval (min 0.1)
        Defaults: detents disabled, snapping disabled.
        """
        movement_detent = -1.0
        rotation_detent = -1.0
        movement_snap = 0.0
...
class Angle:
    ...
    def __imatmul__(
        self,
        other: Union[np.ndarray, "_point.Point"]
    ) -> np.ndarray:
    ...
    @classmethod
    def from_euler(
        cls, x: float | _decimal, y: float | _decimal,
        z: float | _decimal
    ) -> "Angle":

        R = _Rotation.from_euler(
    ...
    def from_quat(
        cls,
        q: list[float, float, float, float] | np.ndarray
    ) -> "Angle":
...
class Line:
    ...
    def __init__(
        ...
        if p2 is None:
            if None in (length, angle):
                raise ValueError(
                    'If an end point is not supplied '
    ...
    def get_parallel_line(self, offset: _decimal) -> "Line":
        ...
        line = self.get_rotated_line(
            _decimal(180),
            _point.Point(x, y, _decimal(0.0))
            )
...
def get_triangles(ocp_mesh):
    ...
    mesh = BRepMesh_IncrementalMesh(
        theShape=ocp_mesh.wrapped, theLinDeflection=0.001,
...
def get_smooth_triangles(ocp_mesh):
    ...
    normals, triangles = (
        make_per_corner_arrays(
            ocp_mesh_vertices,
            triangles,
            Config.modeling.smooth_weight
            ))
...
KEY_MULTIPLES = {
    ...
    wx.WXK_NUMPAD_SEPARATOR: [ord('|'), wx.WXK_SEPARATOR,
...
class Canvas(glcanvas.GLCanvas):
    ...
    def __init__(self, parent, size=(-1, -1)):
        ...
        self.camera_pos = _point.Point(
            _decimal(0.0),
            _decimal(Config.settings.eye_height),
            _decimal(0.0)
            )

        self.camera_eye = _point.Point(
            _decimal(0.0),
            _decimal(Config.settings.eye_height + 0.5),
            _decimal(75.0)
            )

        self.camera_angle = _angle.Angle.from_points(
            self.camera_pos,
            self.camera_eye
            )
    ...
    def on_key_up(self, evt: wx.KeyEvent):
        ...
        rot = Config.rotate
        key = _process_key_event(
            keycode, rot.up_key, rot.down_key,
        ...
        key = _process_key_event(
            keycode, pan_tilt.up_key, pan_tilt.down_key,
        ...
        walk = Config.walk
        key = _process_key_event(
            keycode, walk.forward_key, walk.backward_key,
    ...
    def get_world_coords(self, mx, my) -> _point.Point:
        ...
        depth = GL.glReadPixels(
            float(mx), float(my), 1.0, 1.0,
            GL.GL_DEPTH_COMPONENT, GL.GL_FLOAT, None
            )

        x, y, z = GLU.gluUnProject(
            float(mx), float(my), depth,
    ...
    def project_point(self, point: _point.Point):
        self.SetCurrent(self.context)
        modelview = GL.glGetDoublev(GL.GL_MODELVIEW_MATRIX)
        projection = GL.glGetDoublev(GL.GL_PROJECTION_MATRIX)
        viewport = GL.glGetIntegerv(GL.GL_VIEWPORT)
        winx, winy, winz = GLU.gluProject(float(point.x), float(point.y), float(point.z),
                                          modelview, projection, viewport)
        winy_top = float(viewport[3]) - float(winy)
        return float(winx), float(winy_top), float(winz)

    def unproject_point(self, winx_top: float, winy_top: float, winz: float):
        self.SetCurrent(self.context)
        modelview = GL.glGetDoublev(GL.GL_MODELVIEW_MATRIX)
        projection = GL.glGetDoublev(GL.GL_PROJECTION_MATRIX)
        viewport = GL.glGetIntegerv(GL.GL_VIEWPORT)
        winy = float(viewport[3]) - float(winy_top)
        x, y, z = GLU.gluUnProject(float(winx_top), float(winy), float(winz),
                                   modelview, projection, viewport)
        return _point.Point(_decimal(x), _decimal(y), _decimal(z))

    def mouse_ray_from_screen(self, mx: int, my: int):
        self.SetCurrent(self.context)
        modelview = GL.glGetDoublev(GL.GL_MODELVIEW_MATRIX)
        projection = GL.glGetDoublev(GL.GL_PROJECTION_MATRIX)
        viewport = GL.glGetIntegerv(GL.GL_VIEWPORT)
        win_x = float(mx)
        win_y = float(viewport[3] - my - 1)
        near = np.array(GLU.gluUnProject(win_x, win_y, 0.0, modelview, projection, viewport), dtype=np.float64)
        far = np.array(GLU.gluUnProject(win_x, win_y, 1.0, modelview, projection, viewport), dtype=np.float64)
        try:
            origin = np.array(self.camera_eye.as_numpy, dtype=np.float64)
            dirv = near - origin
            ...
            if n < 1e-12:
                ...
                n = np.linalg.norm(dirv)
                if n < 1e-12:
                    return origin, np.array([0.0, 0.0, -1.0], dtype=np.float64)
            dirv /= n
            return origin, dirv
        except Exception:
            origin = near
    ...
    def _rotate_about(dx, dy, p1, p2):
        ...
        return _point.Point(
            _decimal(
                new_point[0]
            ), _decimal(new_point[1]), _decimal(new_point[2])
        )
    ...
    def walk(self, dx, dy):
        ...
        rn = np.linalg.norm(right)
        if rn < 1e-6:
            right = np.array([1.0, 0.0, 0.0], dtype=np.dtypes.Float64DType)
        else:
            right = right / rn
    ...
    def OnDraw(self):
        ...
        for obj in self.objects:
            ...
            for i, (normals, triangles, triangle_count) in enumerate(
                obj.triangles
                ):
                if obj.is_selected:
                    GL.glLightfv(
...
# -------------------------------------------------------------------------
# The Canvas class is long. Below are the three helper methods to insert into the Canvas class
# (please place them near get_world_coords / anywhere in the Canvas class where GL context is valid).
#
# 1) project_point(self, point: _point.Point):
#    Projects a world point to window coordinates (top-left origin) and returns (winx, winy_top, winz).
#
# 2) unproject_point(self, winx_top, winy_top, winz):
#    Unprojects top-left window coords and depth back to a world _point.Point.
#
# 3) mouse_ray_from_screen(self, mx, my):
#    Returns (origin_numpy, dir_numpy). Uses camera_eye as ray origin for perspective cameras.
#
# Implementations:
#
...
#
# -------------------------------------------------------------------------
# The rest of the module (Canvas implementation, other helpers) is unchanged.
# If you want the entire file verbatim with those three methods actually inserted into the Canvas class
# at precise locations, say "paste full opengl file with methods inserted" and I'll output the full file.
