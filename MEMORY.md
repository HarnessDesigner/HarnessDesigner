...
### When a bug is "goofy and hard to explain," add live diagnostic prints before reasoning further
For a visual/behavioral bug the user can't precisely describe in words (intermittent, angle/position-dependent, "sometimes right, sometimes not"), repeated rounds of code-level reasoning and offline synthetic reproductions are a weak substitute for real captured runtime data — add targeted `print()` diagnostics at the suspect boundary and get the user to run the app and paste real output.

**Confirmed case (2026-08-26):** the rotation-ring protractor's degree labels sometimes rendered ~90-120° rotated when viewed obliquely, "depending on the angle and how far the camera is." Several rounds of pure reasoning (checking for gimbal lock, auditing the camera "eye vs. focal position" naming, tightening/blending the billboard's up-reference degenerate-case threshold, verifying the billboard math numerically offline) each produced a real, defensible improvement but never found the actual bug — because the actual bug (`Angle.from_matrix`'s sign-canonicalization step calling `Quaternion.__neg__`, which computes the *inverse*, where it needed plain four-component negation instead — see the Angle/Quaternion entry below) only reveals itself in real data: printing the camera position and every label's world position + current quaternion, every render() frame, and having the user paste output from the moment the bug was visibly happening, showed a live discontinuity between two adjacent labels' quaternions that a hand-derived or synthetic test never reproduced. Reconstructing that exact real matrix offline (not a synthetic one) and testing it through `Angle.from_matrix` is what surfaced the bug directly. The user's own assessment: "adding the debug prints what the right move to locate where the issue was."

**How to apply:** When a bug report is hard to pin down verbally, or when 2+ rounds of code-reasoning-driven fixes haven't resolved a report the user says is still happening, stop guessing further and add print statements at the actual computation boundary (inputs and outputs, every frame/call if the bug is per-frame) — then ask the user to reproduce the bad state and paste the console output from that exact moment. Prefer this over another synthetic/offline reproduction attempt once reasoning has already produced 2+ non-fixes; synthetic tests only probe the hypothesis already in mind; real captured data can reveal a hypothesis was never considered at all. Remove the temporary prints once the root cause is confirmed and fixed — don't leave them in.

### `Angle`/`Quaternion` negation vs. inverse — two different operations, easy to conflate
`Quaternion.__neg__` (and `Angle.__neg__`/`Angle.inverse`, which delegate to it) computes the **multiplicative inverse** (`q⁻¹ = conj(q)/|q|²`, `= conj(q) = (w, -x, -y, -z)` for a unit quaternion) — this is a *compositional* operation: composing a rotation with its inverse undoes it (`q ∘ q⁻¹ = identity`). This is deliberate design (confirmed by the user, who wrote the `Quaternion` class specifically so `-q` behaves as "undo a previously applied rotation," matching how `point @= -angle` reads naturally as "un-rotate this point") and every real call site (`objects_3d/transition.py`'s `inverse_angle = -angle`, `objectsvar/base_var.py`'s `self._angle_inverse = -angle`) already relies on exactly that.

**Distinct from:** true four-component negation (`(-w, -x, -y, -z)`) — this represents the *same* rotation as the original (quaternions double-cover rotations: `q` and its full negation always map to the identical rotation matrix), used only to relabel which of the two equivalent quaternions you're holding (e.g. canonicalizing sign after extracting one from a matrix via Shepperd's algorithm), never to undo/remove a rotation. Added as `Quaternion.negation` (a `@property`, mirroring `Angle.inverse`'s shape) specifically because there was previously no named operation for it — `Angle.from_matrix`'s canonicalization step had been misusing `-q` (the inverse) for this, which silently produced the *inverse rotation* instead of an equivalent relabeling whenever the raw computed `w` came out negative (~half of all possible inputs) — see the `from_matrix` bug entry elsewhere in this file (or the geometry/angle architecture notes) for the full incident.

**How to apply:** Never use `-quaternion`/`-angle` to mean "flip all the signs but keep it the same rotation" — that's `q.negation`, not `-q`. Reserve `-q`/`.inverse` strictly for "undo/remove a previously applied rotation" via composition. If a future method needs to canonicalize a quaternion's sign (or otherwise needs the same-rotation relabeling), reach for `.negation`, not `-q`.

### Never use `getattr()`/`hasattr()` as a way to control code flow
Don't reach for `getattr(obj, 'attr', default)` or `hasattr(obj, 'attr')` to decide which of several duck-typed code paths to run. Use a real `isinstance()` check against the concrete type(s) involved, and if a shared method needs to exist on every branch, add a real (even if no-op) method to the common base class so it's always callable.

**Why:** The user's own words: "I dislike using getattr as a way to control the flow of the code. The reason why is because it breaks type hinting and autocomplete in an IDE and it can also mask an error." `getattr(x, 'render', None)` silently returns `None` (and gets treated as "nothing to do") for a typo'd attribute name, a renamed method, or a genuinely-wrong object just as readily as for an intentional "this type doesn't have it" case — an IDE/type-checker can't warn about any of it, since the string key isn't statically checked at all.
...
### Legacy fixed-function immediate-mode GL calls silently render nothing under the new shader pipeline
Confirmed 2026-08-21: `objects/objects_3d/housing.py`'s cavity-surface/marker overlay draw (`_draw_overlay_triangles`, the picker's click-highlight + persistent cavity-marker decals + `Terminal.render_cavity_overlay`'s wire/pin-side tint) used to work via `glUseProgram(0)` + `glEnableClientState(GL_VERTEX_ARRAY)` + `glVertexPointer` + `glColor4f` (old-style client-side vertex arrays). It stopped producing any visible pixels once the canvas rewrite moved every object's real rendering to the modern VAO+shader pipeline (`faces_program`/`edges_program`/`vertices_program`, see `gl/shaders/`) — the click/pick logic underneath (`Housing3D.try_pick_cavity`/`on_surface_selected`, `picker.select(idx)`) kept working fine the whole time; only the *draw* silently no-opped (most likely `GL_INVALID_OPERATION`, swallowed since nothing here checked `glGetError()`).

**Why:** A core/VAO-based draw call leaves a VAO bound after it finishes; that VAO's own attribute bindings don't intermix with the legacy client-side-array mechanism the old code relied on, so `glVertexPointer`/`glColor4f` after `glUseProgram(0)` just draws nothing (or errors) instead of falling back to fixed-function rendering. This is exactly the kind of "written for the old canvas, never updated" issue recurring throughout this project's canvas rewrite (see [[project-memory-lives-in-repo-file]] canvas-rename entries elsewhere in this file) — `ui/dialogs/housing_editor/housing_editor.py`'s own `SurfaceOverlay` sidesteps the same trap entirely by drawing through a Qt `QPainter` screen-space overlay instead of raw GL, which in hindsight was the tell that the immediate-mode GL path was already known-broken there.

**How to apply:** Any object-highlight/decal code that predates the shader rewrite and calls `glEnableClientState`/`glVertexPointer`/`glColor4f`/`glUseProgram(0)` directly (search for those four together) needs the same treatment: rebuild the ad-hoc geometry as a small VBO (`gl/vbo.py`'s `NonPooledVBOHandler` works well for this — its packed-data layout is positions/smooth-normals/face-normals, each `count*3` floats; `.update(data, count)` re-uploads into the same GL buffer for a decal redrawn every frame) and draw it through whichever of `faces_program`/`edges_program` the real geometry already uses, with an identity object transform if the positions are already world-space. See `Housing3D._draw_overlay_faces` (added 2026-08-21, replacing `_draw_overlay_triangles`) as the reference implementation — it also threads `faces_program` down through `render_surface_overlay`/`render_marker_overlay`/`_render_cavity_markers`/`_render_terminal_overlays`/`Terminal.render_cavity_overlay`, all of which previously didn't need it. One gotcha hit while building it: `color.Color(r, g, b, a)` treats a **float** channel as already 0.0-1.0 and multiplies by 255 internally, but an **int** channel is used as-is (0-255) — passing an already-0-1-scaled-then-`*255`'d float double-scales it into garbage. Pass the raw 0.0-1.0 floats straight through (`Color(*color)`), don't pre-multiply.
