...
class VBOHandler(metaclass=VBOSingleton):
    ...
    def __init__(
        ...
        *,
        ...
        self.__face_normals = self._normalize_array(face_normals)
        ...
        self.__vert_count = self._normalize_vertex_count(vertex_count_hint, len(self.__vertices))

        if len(self.__vertices) != self.__vert_count:
            self.__vertices = self.__vertices[:self.__vert_count]

        if len(self.__smooth_normals) != self.__vert_count:
            self.__smooth_normals = self.__smooth_normals[:self.__vert_count]
        if len(self.__face_normals) != self.__vert_count:
            self.__face_normals = self.__face_normals[:self.__vert_count]
    ...
    def _normalize_array(data: np.ndarray | None) -> np.ndarray | None:
        ...
        if arr.ndim == 1:
            if arr.size % 3:
                raise ValueError('vertex attribute arrays must have a multiple-of-3 length')
            arr = arr.reshape(-1, 3)
        elif arr.ndim == 2 and arr.shape[1] == 3:
            pass
        else:
            raise ValueError('vertex attribute arrays must have shape (N, 3) or (N*3,)')
