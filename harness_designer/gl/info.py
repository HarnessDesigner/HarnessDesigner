...
def main():
    ...
    print('GFX Manufacturer:', vendor)

    renderer = GL.glGetString(GL.GL_RENDERER).decode()
    print('GFX Adapter:', renderer)
    ...
    while True:
        try:
            res = GL.glGetStringi(GL.GL_EXTENSIONS, i)
            plugins.append(res.decode())
            i += 1
        except:
            break

    print('Supported plugins:', ', '.join(plugins))