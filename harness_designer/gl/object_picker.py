...
def find_object(mouse_pos, camera: Union["_camera3d.Camera", "_camera2d.Camera"],
    ...
    *current_selection* -- lets repeated clicks cycle through a stack
    ...
    # hit_test yields (slot index, t_hit) pairs, nearest first -- resolve
    # each back to the live object that owns the slot (None if it was
    # already garbage collected).
    candidates = [obj for obj in (pool.resolve(index) for index, _ in pool_hits)
                  if obj is not None]
    ...
    # see BaseVar._pick_priority), checked before the costlier step3 test.
    hits = [wrapped for wrapped in candidates
...
def find_object_in_list(mouse_pos, scene_objects,
    ...
    *current_selection* -- lets repeated clicks cycle through a stack of
    ...
    Original list-based picker, kept for callers that pick against a
    private candidate list that isn't registered with the canvas's bounds
    manager (the rotation rings' tick facades -- see
    ``rotation_handlers.rotation_ring.outer_ring.OuterRing.pick_tick``);
    everything registered with a canvas goes through :func:`find_object`.
