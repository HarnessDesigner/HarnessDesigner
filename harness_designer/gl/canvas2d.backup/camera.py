...
class Camera:
    ...
    def _projection_matrix(self) -> np.ndarray:
        ...
        ``gl.canvas2d.canvas.Canvas._projection_matrix``.
        """

        width, height = self.size
        world_per_pixel = self.distance / 1000.0
    ...
    def invalidate_views(self) -> None:
        """Force the next :meth:`_update_views` call to actually re-read
        ``.projection``/``.modelview``/``.viewport`` from live GL state,
        instead of trusting the cached values from whenever ``_is_dirty``
        was last cleared.

        ``Canvas.resizeGL`` must call this on every resize -- unlike
        camera movement (pan/zoom, which already invalidate themselves
        via ``_on_focal_position_changed``/their own setters), nothing
        else ever notices a viewport-size change, so without this the
        picking matrices silently drift out of sync with the real GL
        viewport/``glOrtho`` bounds after the first resize, even though
        rendering (which reads ``Canvas.size`` and re-runs
        ``_setup_projection()`` fresh every frame) keeps looking correct.
        The visible symptom is every click missing: pan/zoom still work
        (they don't depend on this cache), but ``gl.object_picker.
        find_object``'s ray math is built from the stale matrices.
        """
        self._is_dirty = True
