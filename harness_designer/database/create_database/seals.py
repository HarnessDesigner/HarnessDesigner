...
def add_pjt_seal(con, project_id, part_id, point3d_id=None, housing_id=None,
    ...
    con.execute('INSERT INTO pjt_seals (id, part_id, point3d_id, housing_id, '
                'terminal_id, name, notes, quat3d, angle3d, is_visible3d) '
                'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
                (new_id, part_id, point3d_id, housing_id, terminal_id, name,
...
table = _con.SQLTable(
    ...
    _con.UUIDField('model3d_id', default='NULL',
                  references=_con.SQLFieldReference(_models3d.table,
                                                    _models3d.id_field,
                                                    on_update=_con.REFERENCE_NO_ACTION)),
    # No sentinel default (unlike housings.seal_type_id, which is
    # genuinely optional) -- an actual seal part's own type is required,
    ...
    # (seal_types no longer even has one -- see
