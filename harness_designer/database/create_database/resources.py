...
def _download_image(con, url):
    ...
    image_path = _settings.get_setting(con, 'image_path')
...
def add_resource(con, image_type, path):
    if not path:
        return None
    ...
    if path.startswith('http'):
        image_path = _download_image(con, path)
...
def get_resource_id(con, path, type="UNKNOWN"):  # NOQA
    if not path:
        return None

    con.execute(f'SELECT id FROM resources WHERE path="{path}";')
    res = con.fetchall()
    ...
    if not res:
        ...
        con.execute('INSERT INTO resources (path, file_type_id) VALUES (?, ?);', (path, type))
