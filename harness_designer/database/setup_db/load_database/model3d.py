...
def download_model(con, cur, url):
    time.sleep(0.05)
    ...
    for key, value in headers.items():
        if key.lower() == 'content-type':
            ...
            if content_type == 'model/step+zip':
                ...
                res = cur.execute('SELECT extension FROM file_types WHERE is_model=1;')
                res = res.fetchall()
                ...
                db_id = _file_types.get_file_type(con, cur, mimetype=content_type)
                if db_id is None:
                    res = cur.execute('SELECT extension FROM file_types WHERE is_model=1;')
                    res = res.fetchall()
                    for row in res:
                    ...
                    db_id = _file_types.get_file_type(con, cur, ext, content_type)
        ...
        db_id = _file_types.get_file_type(con, cur, ext)
        data = response.content
        return db_id, data
...
def add_model3d(con, cur, path):
    ...
    if path.startswith('http'):
        file_type_id, data = download_model(con, cur, path)
        ...
        rows = cur.execute(f'SELECT id FROM file_types WHERE extension={ext};')
    ...
    rows = cur.execute(f'SELECT extension FROM file_types WHERE id={file_type_id};')
    ext = rows.fetchall()[0][0]

    uuid_ = str(uuid.uuid4())

    save_path = _settings.get_setting(con, cur, 'model_path')
