...
class ProtectionsTable(TableBase):
    """Represent a protections table in :mod:`harness_designer.database.global_db.protection`.
    ...
    def __getitem__(self, item) -> "Protection":
        ...
        :type item: UNKNOWN
        :returns: Return value. UNKNOWN details.
        :rtype: :class:`Protection`
        :raises KeyError: Raised when the operation cannot be completed.
        :raises IndexError: Raised when the operation cannot be completed.
        """
        if isinstance(item, (int, bytes)):
            if item in Protection or item in self:
...
class Protection(EntryBase, NameMixin):
    """Represent a protection in :mod:`harness_designer.database.global_db.protection`.
