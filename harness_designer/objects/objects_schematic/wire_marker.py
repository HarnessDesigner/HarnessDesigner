...
class WireMarker(_base_schematic.BaseSchematic):
    ...
    def _segment_for_point(
        ...
        *wire*'s current path -- start, through each interior waypoint in
        ...
        exactly, using the 2D waypoint chain (``waypoints2d``) instead.
        """
        points = [wire.start_position2d]
        for waypoint in wire.waypoints2d:
            points.append(waypoint.point)
        points.append(wire.stop_position2d)
    ...
    def _update_position(self, position: _point.Point):
        ...
        callback in __init__ *and* BaseSchematic binds it to the marker's own
