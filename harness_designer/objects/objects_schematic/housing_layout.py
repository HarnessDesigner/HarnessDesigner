...
def _occupied_rects(project: "_project.Project", housing: "_housing_obj.Housing"
    ...
    for other in project.housings:
        ...
        bounds = other.objschematic.get_bounds()
        if bounds is None:
            continue
    ...
    for wire in project.wires:
        ...
        bounds = wire.objschematic.get_bounds()
        if bounds is None:
            continue
...
@_check_types.do
def _centroid(project: "_project.Project", housing: "_housing_obj.Housing") -> tuple[float, float]:
    others = [h for h in project.housings if h is not housing and h.objschematic.get_bounds() is not None]
    ...
    zs = []
    for other in others:
        min_x, min_z, max_x, max_z = other.objschematic.get_bounds()
