...
class CPALock(_base_3d.Base3D):
    ...
    def start_add(
        ...
        compat_cpa_locks = [] if housing is None else housing.db_obj.part.compat_cpas_array
        ...
        if part_id is None:
            dlg = _part_search.SearchDialog(
                ...
                initial_results=compat_cpa_locks)
        ...
        part = ptables.global_db.cpa_locks_table[part_id]
        ...
        if housing is None:
            ...
            db_obj = ptables.pjt_cpa_locks_table.insert(part_id, name, pos_obj.db_id, None)
        else:
            pos_id = housing.db_obj.cpa_lock_position3d_id
            db_obj = ptables.pjt_cpa_locks_table.insert(
                part_id, name, pos_id, housing.db_obj.db_id)

        facade = _cpa_lock_facade.CPALock(mainframe, db_obj)
        facade.identify(preview_material)

        if housing is not None:
            _handler_base.HandlerBase.set_angle_from_housing(facade, housing)

        handler = _add_cpa_lock.CPALock(canvas, facade, housing, project_housings)
    ...
    def handle_interaction(
        ...
        """
        from ...add_handlers.editor_3d import cpa_lock as _add_cpa_lock  # NOQA -- avoid a cycle at import time

        if isinstance(self._active_handler, _add_cpa_lock.CPALock):
