...
(one per Euler axis, :class:`~.rotation_ring.torus_ring.TorusRing`).
Clicking anywhere on one shows that axis's protractor -- an object-space
:class:`~.rotation_ring.inner_ring.InnerRing` (spins with the object,
grab-and-drag anywhere on its band free-rotates about this axis) and a
world-space :class:`~.rotation_ring.outer_ring.OuterRing` (own spin
fixed, hover a tick to highlight it, click to snap the object's local
zero to that exact angle) -- and dims the other two axes' torus rings.
The Euler value is the only thing ever written -- the quaternion is
rebuilt from it by :class:`_angle.Angle` (see the Euler rule in
CODEBASE_MAP.md).
