...
class AddTPALockHandler(_handler_base.HandlerBase):
    ...
    def finalize(self, mouse_pos: _point.Point):
        ...
        tpa_lock = self.mainframe.project.ptables.pjt_tpa_locks_table.insert(
            self.part_id, position.db_id[:-2], housing_id=housing_id)

        angle_delta = angle - tpa_lock.angle3d
        angle = tpa_lock.angle3d
        angle += angle_delta

        tpa_lock = _tpa_lock.TPALock(self.mainframe, tpa_lock)

        self.mainframe.project.add_tpa_lock(tpa_lock)
