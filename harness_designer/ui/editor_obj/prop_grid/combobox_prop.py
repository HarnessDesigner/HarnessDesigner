...
class ComboboxCtrl(wx.Panel):

    def __init__(self, parent, value, choices, pos, size):
        ...
        self.__ctrl = _autocomplete_combobox.AutoCompleteComboBox(
    ...
    def InsertItem(self, label, index):
        items = self.ctrl.GetItems()
        value = self.GetValue()
        items.insert(index, label)
        self.__ctrl.SetItems(items)
        self.SetValue(value)
...
class ComboboxEditor(wxpg.PGEditor):
    ...
    def CreateControls(self, propgrid, prop, pos, sz):
        ...
        ctrl = ComboboxCtrl(propgrid.GetPanel(), value=value, choices=choices, pos=pos, size=sz)
...
class ComboboxProperty(wxpg.PGProperty):
    ...
    def DoGetEditorClass(self):
        return wxpg.PropertyGridInterface.GetEditorByName("Combobox")
