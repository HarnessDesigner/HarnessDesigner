...
"""Single-click housing placement for the pegboard editor.

Mirrors ``add_handlers.editor_3d.housing.Housing``/``add_handlers.
editor_schematic.housing.Housing`` -- a housing floats freely here too,
following the cursor across the pegboard's own permanently top-down
X/Z plane (see ``objects.objects_pegboard.base_pegboard.BasePegboard.
drag``'s own docstring for why that plane is fixed) until the first
...
class Housing(_base.AddHandlerBase):
    ...
    def _follow(self, mouse_pos: _point.Point) -> None:
        ...
        position = self.target.objpegboard.position
