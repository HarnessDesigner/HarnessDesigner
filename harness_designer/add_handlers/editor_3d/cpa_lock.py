...
"""Housing-snapping CPA lock placement for the 3D editor.

Ported from ``handlers.cpa_lock_handler.AddCPALockHandler`` -- same
...
class CPALock(_base.AddHandlerBase):
    """Housing-snapping CPA lock placement -- see the module docstring."""
    ...
    def snap_pool(self):
        ...
        for housing in self._project_housings:
            ...
            positions.append(housing.db_obj.cpa_lock_position3d)
    ...
    def _finalize(self) -> None:
        if self._housing is None:
            ...
            self._snapped.db_obj.cpa_lock_position3d.attach(self.target.db_obj.position3d)
        ...
        self._finalized = True
        self.mainframe.project.add_cpa_lock(self.target)
