...
is the simplest possible add session: every move just follows the
cursor across the camera's focal plane, and the first (non-drag) click
locks it in.
"""
...
class Housing(_base.AddHandlerBase):
    ...
    def _follow(self, mouse_pos: _point.Point) -> None:
        ...
        focal_position = self.camera.get_position_on_focal_plane(mouse_pos)
        position += focal_position - position
