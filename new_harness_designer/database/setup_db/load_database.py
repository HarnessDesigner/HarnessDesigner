...
def _build_wire_markers(con, cur):
    ...
    mfg_id = get_mfg_id(con, cur, 'Milspecwiring.com')
    res = []
...
def _build_wires(con, cur):
    ...
    values = []
    mfg_id = get_mfg_id(con, cur, 'TE')

    family_id = get_family_id(con, cur, 'Tefzel', mfg_id)
...
def add_cover(con, cur, part_number, description, mfg, series, length, width, height, color,
              pins, direction, min_temp, max_temp, image, cad, family=''):
    ...
    cur.execute('INSERT INTO covers (part_number, description, mfg_id, series_id, color_id, '
                'image_id, direction_id, min_temp_id, max_temp_id, length, width, '
                'height, pins) '
                'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
                (part_number, description, mfg_id, series_id, color_id, image_id, direction_id,
                 min_temp_id, max_temp_id, length, width, height, pins))

    con.commit()


def add_cpa_lock(con, cur, part_number, description, mfg, series, family, length, width, height, color,
    ...
    cur.execute('INSERT INTO cpa_locks (part_number, description, mfg_id, series_id, family_id, '
                'color_id, image_id, cad_id, terminal_size, min_temp_id, max_temp_id, '
                'length, width, height, pins) '
                'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
                (part_number, description, mfg_id, series_id, family_id, color_id, image_id,
...
def add_tpa_lock(con, cur, part_number, description, mfg, series, family, length, width, height, color,
                 pins, terminal_size, min_temp, max_temp, image, cad):
    ...
    cur.execute('INSERT INTO tpa_locks (part_number, description, mfg_id, series_id, family_id, '
                'color_id, image_id, cad_id, terminal_size, min_temp_id, max_temp_id, '
                'length, width, height, pins) '
                'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
                (part_number, description, mfg_id, series_id, family_id, color_id, image_id,
...
def add_seal(con, cur, part_number, description, mfg, series, type, length, o_dia, i_dia, color,
    ...
    cur.execute('INSERT INTO seals (part_number, description, mfg_id, series_id, type_id, '
                'color_id, image_id, cad_id, lubricant, min_temp_id, max_temp_id, '
                'length, o_dia, i_dia, hardness, wire_dia_min, wire_dia_max) '
                'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
                (part_number, description, mfg_id, series_id, type_id, color_id, image_id,
...
def add_terminal(con, cur, part_number, description, mfg, series, cavity_lock, wire_dia_min,
    ...
    cur.execute('INSERT INTO terminals (part_number, description, mfg_id, series_id, plating_id, '
                ...
                'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
                (part_number, description, mfg_id, series_id, plating_id, image_id,
...
def add_housing(con, cur, part_number, description, mfg, family, series, num_pins, rows,
    ...
    cur.execute('INSERT INTO housings (part_number, description, mfg_id, family_id, series_id, '
...
def get_model_type(con, cur, extension):
    if not extension:
        return 0

    extension = extension.lower()
    res = cur.execute(f'SELECT id FROM model_types WHERE extension="{extension}";').fetchall()
...
def add_model_types(con, cur):
    res = cur.execute('SELECT id FROM model_types WHERE id=0;')
    if res.fetchall():
        return

    data = (
        (0, None, 'Not Set'),
        (1, 'mdl', '3D GameStudio Model'),
        (2, 'hmp', '3D GameStudio Terrain'),
        (3, '3mf', '3D Manufacturing Format'),
        (4, '3ds', '3D Studio Max 3DS'),
        (5, 'ase', '3D Studio Max ASE'),
        (6, 'ac', 'AC3D'),
        (7, 'ac3d', 'AC3D'),
        (8, 'dxf', 'AutoCAD DXF'),
        (9, 'bvh', 'Biovision BVH'),
        (10, 'blend', 'Blender'),
        (11, 'csm', 'CharacterStudio Motion'),
        (12, 'dae', 'Collada'),
        (13, 'x', 'DirectX X'),
        (14, 'md5mesh', 'Doom 3'),
        (15, 'md5anim', 'Doom 3'),
        ...
        (18, 'glTF', 'glTF'),
        (19, 'ifc', 'IFC-STEP'),
        (20, 'igs', 'Initial Graphics Exchange Specification'),
        (21, 'iges', 'Initial Graphics Exchange Specification'),
        (22, 'irrmesh', 'Irrlicht Mesh'),
        (23, 'xml', 'Irrlicht Mesh'),
        (24, 'irr', 'Irrlicht Scene'),
        (25, 'lwo', 'LightWave Model'),
        (26, 'lws', 'LightWave Scene'),
        (27, 'ms3d', 'Milkshape 3D'),
        (28, 'lxo', 'Modo Model'),
        (29, 'nff', 'Neutral File Format'),
        (30, 'off', 'Object File Format'),
        (31, 'mesh.xml', 'Ogre'),
        (32, 'skeleton.xml', 'Ogre'),
        (33, 'material', 'Ogre'),
        (34, 'ogex', 'OpenGEX-Fomat'),
        (35, 'mdl', 'Quake I'),
        (36, 'md2', 'Quake II'),
        (37, 'md3', 'Quake III'),
        (38, 'pk3', 'Quake 3 BSP'),
        (39, 'q3o', 'Quick3D'),
        (40, 'q3s', 'Quick3D'),
        (41, 'raw', 'Raw Triangles'),
        (42, 'mdc', 'RtCW'),
        (43, 'nff', 'Sense8 WorldToolkit'),
        (44, 'stp', 'Standard for the Exchange of Product Data'),
        (45, 'step', 'Standard for the Exchange of Product Data'),
        (46, 'ply', 'Polygon File Format'),
        (47, 'stl', 'Stereolithography'),
        (48, 'ter', 'Terragen Terrain'),
        (49, 'cob', 'TrueSpace'),
        (50, 'scn', 'TrueSpace'),
        (51, 'smd', 'Valve Model'),
        (52, 'vta', 'Valve Model'),
        (53, 'wrl', 'Virtual Reality Modeling Language'),
        (54, 'obj', 'Wavefront Object'),
        (55, 'xgl', 'XGL-3D-Format'),
        (56, 'amf', 'Additive manufacturing file format'),
        (57, 'assbin', 'Allegorithmic Substance Painter Scene within Document'),
        (58, 'b3d', 'OpenBVE 3D'),
        (59, 'iqm', 'Inter-Quake Model'),
        (60, 'ndo', '3D Low-polygon Modeler'),
        (61, 'q3d', 'Quest3D'),
        (62, 'sib', 'Silo Model Format'),
        (63, 'x3d', 'Extensible 3D'),
        (64, 'x3db', 'Extensible 3D'),
        (65, 'x3dz', 'Extensible 3D'),
        (66, 'x3dbz', 'Extensible 3D'),
        (67, 'glb', 'GLB'),
        (68, 'pmx', 'MikuMikuDance Format'),
        (69, 'pmd', 'MikuMikuDance Format')
    )

    splash.SetText(f'Adding model types to db [0 | {len(data)}]...')
    cur.executemany('INSERT INTO model_types (id, extension, name) VALUES (?, ?, ?);', data)
    splash.SetText(f'Adding model types to db [{len(data)} | {len(data)}]...')
    con.commit()
