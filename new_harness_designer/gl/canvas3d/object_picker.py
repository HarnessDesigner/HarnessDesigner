...
@_debug.logfunc
def _aabb_screen_bbox_and_depth(bboxes, mv, pj, viewport):
    """
    ...
    for corner in bboxes:
        proj = _project_point_world(corner, mv, pj, viewport)
        if proj is None:
            continue
...
def _pick_candidates_at_mouse(mx, my, scene_objects, mv=None, pj=None, viewport=None,
                             mouse_is_top_left=True, tol_pixels=3.0):  # NOQA
    """
    ...
    for obj in scene_objects:
        for (minx, miny, maxx, maxy), depth in (
            _aabb_screen_bbox_and_depth(obj.obj3d.obb, mv, pj, viewport)
        ):
...
def find_object(mouse_pos, scene_objects):
    ...
    candidates = _pick_candidates_at_mouse(mx, my, scene_objects, mv, pj, vp)
