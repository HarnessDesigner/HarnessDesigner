...
def activate():
    ...
    invokes a native compiler (cl.exe via Cython's build_ext, or cmake/ninja
    for the assimp build) — pyMSVC mutates os.environ (PATH/INCLUDE/LIB), so
    it only affects the process it runs in, not sibling CI steps.
    """
